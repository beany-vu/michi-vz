...
export const AutoRotatingXAxisLabels = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          "26 monthly periods × 2 series. At this density horizontal labels would have to be dropped — instead the chart measures each label against the band width and tilts them to -45° when they don't fit, keeping every month visible. Adjacent rotated labels graze each other but the text trails down-left from each anchor so they remain readable. The bottom margin auto-expands when rotation triggers so the canvas-drawn series abbreviation row stays above the dot and the rotated labels below it never get clipped. Pass `xAxisLabelMode=\"horizontal\"` to opt out and restore the legacy skip-with-dots behaviour.",
      },
    },
  },
};
